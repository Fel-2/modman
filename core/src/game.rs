...
pub const CATALOG: &[GameSpec] = &[
    use_gamedir("skyrimse",  "Skyrim Special Edition", 489830,  "Data",       true,  Engine::Bethesda,  "Skyrim Special Edition", LoadOrderKind::PluginsTxt,   "skyrimspecialedition"),
    use_gamedir("skyrim",    "Skyrim",                 72850,   "Data",       true,  Engine::Bethesda,  "Skyrim",                 LoadOrderKind::PluginsTxt,   "skyrim"),
    use_gamedir("fallout4",  "Fallout 4",              377160,  "Data",       true,  Engine::Bethesda,  "Fallout4",               LoadOrderKind::PluginsTxt,   "fallout4"),
    use_gamedir("falloutnv", "Fallout: New Vegas",     22380,   "Data",       true,  Engine::Bethesda,  "FalloutNV",              LoadOrderKind::PluginsTxt,   "newvegas"),
    use_gamedir("fallout3",  "Fallout 3",              22300,   "Data",       true,  Engine::Bethesda,  "Fallout3",               LoadOrderKind::PluginsTxt,   "fallout3"),
    use_gamedir("oblivion",  "Oblivion",               22330,   "Data",       true,  Engine::Bethesda,  "Oblivion",               LoadOrderKind::PluginsTxt,   "oblivion"),
    use_gamedir("morrowind", "Morrowind",              22320,   "Data Files", true,  Engine::Bethesda,  "",                       LoadOrderKind::MorrowindIni, "morrowind"),
    use_gamedir("starfield", "Starfield",              1716740, "Data",       true,  Engine::Bethesda,  "Starfield",              LoadOrderKind::PluginsTxt,   "starfield"),
    use_gamedir("cyberpunk", "Cyberpunk 2077",         1091500, "",           true,  Engine::Cyberpunk, "",                       LoadOrderKind::None,         "cyberpunk2077"),
    // Folder-per-mod games — wrapper folder is the mod, do not flatten.
    use_gamedir("rimworld",  "RimWorld",               294100,  "Mods",       false, Engine::RimWorld,  "",                       LoadOrderKind::None,         "rimworld"),
    use_gamedir("stardew",   "Stardew Valley",         413150,  "Mods",       false, Engine::Stardew,   "",                       LoadOrderKind::None,         "stardewvalley"),
    // Generic Unity / BepInEx — manual-add (no fixed appid).
    use_gamedir("unity",     "Generic Unity (BepInEx)", 0,      "BepInEx/plugins", false, Engine::Unity, "",                     LoadOrderKind::None,         ""),
    // Paradox — mods deploy to Documents/.../mod inside the prefix.
    GameSpec { id: "ck3", name: "Crusader Kings III", steam_appid: 1158310, deploy: DeployTarget::PrefixDocs("Paradox Interactive/Crusader Kings III/mod"), flatten: false, engine: Engine::Paradox, appdata: "", load_order: LoadOrderKind::None, nexus_domain: "crusaderkings3" },
    GameSpec { id: "ck2", name: "Crusader Kings II",  steam_appid: 203770,  deploy: DeployTarget::PrefixDocs("Paradox Interactive/Crusader Kings II/mod"),  flatten: false, engine: Engine::Paradox, appdata: "", load_order: LoadOrderKind::None, nexus_domain: "crusaderkings2" },
];
    ...
    for entry in entries.flatten() {
        ...
        if let Some(install) = resolve_install_dir(&steamapps, &entry.path()) {
            if out.iter().all(|g| g.path != install) {
                out.push(InstalledGame { spec, path: install });
            }
        }
    }
}
